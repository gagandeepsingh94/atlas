---
name: domain-models
description: Deeply explore a service repo and generate docs/domain-models.md — a comprehensive human-readable domain model reference with Mermaid diagrams. Automatically discovers related transport/proto repos in sibling directories. Use when onboarding to a new service, preparing for a cross-service change, or documenting the data model of a repo.
argument-hint: [repo-path-or-name]
---

# Domain Models Skill

Generate `docs/domain-models.md` for a service by deeply exploring its domain models, cross-service dependencies, and data flows. Include Mermaid diagrams for every key concept.
...
Create the file at `TARGET_REPO/docs/domain-models.md`. Structure it as follows. Include only sections that have content — skip empty sections rather than writing placeholders.
